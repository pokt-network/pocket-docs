...
- The `pocketd` binary installed and configured. See
  [pocketd CLI](/pocketd/) for installation instructions.
...
```bash
pocketd query gateway params --network=beta
```
...
- `--network` — The target network (`local`, `beta`, or `main`). Sets the
  chain ID and endpoints for you.
...
```bash
pocketd query tx <tx_hash> --network=beta
```
