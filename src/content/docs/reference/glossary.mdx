...
An API service registered onchain that Suppliers can offer and Applications can consume. Each service has a unique ID, a human-readable name, a `compute_units_per_relay` value, and an owner address. Services can optionally include metadata (up to 256 KiB). See [Service Management](/services/management).
...
A node that synchronizes and stores blockchain data but does not participate in consensus (unlike a Validator). Full Nodes serve RPC queries and can be used as backend nodes for Gateways and RelayMiners. See [pocketd CLI](/pocketd/).
