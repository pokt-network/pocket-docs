...
<Callout type="info">
  The `--experimental-metadata-*` flags still work but are deprecated aliases. Metadata is now published as a structured service card with `--card-file` or `--card-base64`, validated with `pocketd tx service validate-card`, and read back with `pocketd query service card`. The end-to-end walkthrough is in [Register a Service](/services/register).
</Callout>
...
For more about staking suppliers against services, see [Supplier Staking](/node-operators/supplier-staking). For account setup and CLI basics, see the [pocketd CLI](/pocketd/) section.
