...
```bash
pocketd query bank balances <your_app_address> --network=beta
```
...
```bash
pocketd query application params --network=beta
```
...
| `--network` | Network to broadcast the transaction to: `local`, `beta`, or `main`. Sets the chain ID and endpoints for you. |
...
```bash
pocketd query application show-application pokt1yourappaddress123 --network=beta
```
...
```bash
pocketd query shared params --network=beta
```
...
```bash
pocketd query application list-application --network=beta
```
